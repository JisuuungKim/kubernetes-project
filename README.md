...
이 프로젝트는 실제 Slack API를 직접 부하테스트하지 않습니다. 대신 Slack 조회 흐름을 Kubernetes 내부에 배포한 외부 mock upstream으로 치환해 같은 아키텍처 패턴을 재현합니다.

- 실제 시나리오의 `Slack 공지/알림 메시지 조회 시스템` -> `traffic-app`
- 실제 시나리오의 `Slack API 조회` -> `meme-content-api`를 외부 mock upstream처럼 노출한 `mock-slack` Ingress
...
- `mock-slack` Ingress 뒤의 `meme-content-api /meme`를 호출해 공지 문구 대체 콘텐츠 조회
- 결과를 합쳐 사용자에게 응답

즉, 테스트 흐름은 아래와 같습니다.

`Client -> traffic-platform Ingress(/api) -> traffic-app -> traffic-counter-api(/hit, Redis) + meme-mock-platform Ingress(/mock-slack) -> meme-content-api(/meme) -> Response`

## 아키텍처

```mermaid
flowchart LR
    A["Client / Load Generator"] --> B["traffic-platform Ingress<br/>/api, /redis"]
    A --> H["meme-mock-platform Ingress<br/>/mock-slack"]

    B --> C["traffic-app"]
    B --> G["redis-admin (optional)"]

    C --> D["traffic-counter-api"]
    D --> F["Redis"]

    C --> H
    H --> E["meme-content-api<br/>Mock Slack upstream"]
```

핵심 포인트는 메인 진입점과 mock upstream 진입점을 Ingress로 분리했다는 점입니다.
...
| `meme-content-api` | Slack API를 대체하는 외부 mock upstream 역할 서비스 | `/meme`, `/health` |
...
- `GET /notice`
  ...
  - 그 다음 `meme-mock-platform` Ingress를 통해 외부 mock upstream(`/mock-slack -> meme-content-api /meme`) 호출
  - 두 결과를 합쳐 최종 응답 반환
- `GET /notice/track`
  - `traffic-counter-api /hit`만 호출
  - counter 서비스만 별도 부하테스트할 때 사용
- `GET /notice/message`
  - `meme-mock-platform` Ingress를 통해 외부 mock upstream(`/mock-slack -> meme-content-api /meme`)만 호출
...
Ingress는 역할별로 두 개를 사용합니다.

- `traffic-platform`
  - `/api(/|$)(.*)` -> `traffic-app:8000`
  - `/redis(/|$)(.*)` -> `redis-admin:8081`
- `meme-mock-platform`
  - `/mock-slack(/|$)(.*)` -> `meme-content-api:9100`
...
- `/mock-slack/meme` -> 내부적으로 `meme-content-api /meme`
...
- `meme-content-api`는 별도 Ingress 뒤에 둔 Slack 대체 mock upstream
- `traffic-counter-api`는 요청 수 기록 전용 내부 API
- 메인 진입점(`traffic-platform`)과 mock upstream 진입점(`meme-mock-platform`)을 분리
- Redis는 카운터 저장소
