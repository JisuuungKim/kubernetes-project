...
```mermaid
flowchart LR
    A["Load Generator<br/>Python / k6 / Locust"] --> B["Ingress<br/>/api"]
...
- `GET /notice`
  - 내부적으로 `traffic-counter-api /hit` 호출
  - 내부적으로 `meme-content-api /meme` 호출
  - 두 결과를 합쳐 최종 응답 반환
- `GET /stats`
...
├── loadtest/                    # k6, Locust, Python 부하테스트 스크립트
...
- `LOCAL_PORT` 기본값 `8000`
- `TARGET_URL` 기본값 `http://127.0.0.1:8000/notice`
- `CONCURRENCY` 기본값 `100`
- `DURATION` 기본값 `120`
- `THINK_TIME` 기본값 `0.0`
...
```bash
k6 run loadtest/k6-traffic-app.js -e BASE_URL=http://localhost:8000
```
...
짧은 검증 부하를 다음과 같이 실행했습니다.

```bash
CONCURRENCY=10 DURATION=15 ./scripts/run_portforward_load_test.sh
```
...
- 성공률: `100.00%`
- RPS: `385.07`
- p95 latency: `56.94 ms`
- HPA 최대 CPU: `4%`
- HPA 확장 없음
- Redis hit 증가량: `5776`

이 결과는 낮은 부하에서는 시스템이 안정적으로 동작하지만, HPA 임계치인 CPU 50%에는 도달하지 않았다는 의미입니다.
