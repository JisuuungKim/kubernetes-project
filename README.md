...
`Client -> Ingress(/api) -> traffic-app -> traffic-counter-api(/hit, Redis) + mock-slack Ingress -> meme-content-api(/meme) -> Response`
...
flowchart LR
    ...
    C --> H["mock-slack Ingress<br/>/mock-slack"]
    H --> E["meme-content-api<br/>FastAPI /meme"]
    D --> F["Redis"]
...
- 외부 노출:
  ...
  - `/mock-slack` -> `meme-content-api` mock upstream
- 내부 전용:
  - `traffic-counter-api`
  - `redis-service`
...
| `meme-content-api` | Slack API를 대체하는 외부 mock upstream | `/meme`, `/health` |
...
- `GET /notice`
  ...
  - 그 다음 외부 mock upstream(`/mock-slack -> meme-content-api /meme`) 호출
  - 두 결과를 합쳐 최종 응답 반환
- `GET /notice/track`
  - `traffic-counter-api /hit`만 호출
  - counter 서비스만 별도 부하테스트할 때 사용
- `GET /notice/message`
  - 외부 mock upstream(`/mock-slack -> meme-content-api /meme`)만 호출
...
- `meme-content-api`는 Slack API를 대신하는 외부 mock upstream
- `traffic-counter-api`는 요청 수 기록 전용 내부 API
- Ingress는 `/api`, `/mock-slack`를 외부에 노출
- Redis는 카운터 저장소
